...
- 위치 검색을 허용하시면 인근 대여소를 제공합니다.

<h4>2. 도착 대여소를 선택하세요.</h4>

- 가고 싶은 지역이나 방향을 생각한 다음 대여소를 선택하시면 알맞은 여행지를 찾으실 수 있습니다.
...
<a href="https://naver.com" target="_blank"><img src="ReadMeImg/seoulIcon.svg"  ></a>

## 활용 데이터 <a name="dataset"></a>

- 서울 공공데이터 포털에서 확보한 `2021년 따릉이 대여기록` 데이터 활용
- 해당 데이터는 Raw 3,000만건 6개 column(`자전거 번호`, `대여날짜`, `대여 대여소 번호`, `반납 대여소 번호`,`대여 시간`, `이동거리`)로 구성
- 전처리 과정에서 대여 시간과 이동 거리를 활용해 `분당 이동거리`라는 새로운 Feature을 만들어 오류가 있는 대여 기록 300만건을 제거, 총 2,700만건의 데이터로 분석을 진행
- 따릉이 대여소 관련 일반사항
...
<a href="https://naver.com" target="_blank">Analysis</a> &nbsp; | &nbsp; <a href="https://github.com/yangoos57/seoulBikeProject/blob/main/backend/bikeTourUtils.py" target="_blank">Backend</a> &nbsp; | &nbsp; <a href="https://github.com/yangoos57/seoulBikeProject/tree/main/frontend/src/components/bt" target="_blank">Frontend</a>

<br/>

## 문제정의 및 해결 <a name="development"></a>

### 상관관계 분석을 통해 여행 목적에 적합하지 않은 대여소 제거

**문제 정의**

- 오르막길이 있는 경로를 이용할 경우 체력적인 부담으로 인해 자전거 여행을 온전히 즐기지 못할 가능성 존재
- 경로 상 오르막길이 있을 것으로 판단되는 대여소를 식별해 제거 할 필요성 인식

**문제 해결**

- 대여소 간 대여 비율 차이가 발생하는 원인을 분석해 오르막길이 있을것으로 판단되는 대여소 식별
- A 대여소에서 B 대여소로 이동 기록은 200건인 반면 B 대여소에서 A 대여소로의 이동 기록은 50건으로 4배의 비율 차이가 발생
    <div align='center'>
        <img width='550px' src='ReadMeImg/bikeRentalRate.png'>
    </div>
- 같은 경로를 이용함에도 비율 차이가 발생하는 원인을 찾기 위해 EDA를 수행한 결과, 대여 비율과 대여소 고도 차이 간 상관 관계 식별
- 상관관계 식별을 위해 사용한 식은 다음과 같음
- 비율 = $log_{2}(\frac{A→B 대여건수}{B→A 대여건수})$ &nbsp;|&nbsp; 고도차 = 출발지 고도 - 목적지 고도
- 초록색 영역은 출발 대여소 → 도착 대여소 경로가 오르막인 경우를, 주황색 영역은 출발 대여소 → 도착 대여소 경로가 내리막인 경우를 의미
   <div align='center'>
       <img width='550px' src='ReadMeImg/rateAndHeight.png'>
   </div>
- 그래프를 통해 오르막 영역에서는 고도차이가 작아질수록 비율이 증가하고, 내리막 영역에서는 고도차이가 커질수록 비율이 증가하는 양상을 확인 할 수 있음
- 대여소 간 비율을 계산한 뒤 비율이 -0.4 미만인 경우, 경로 상 오르막길이 있을것으로 판단해 해당 대여소 제거

<br/>

### 2. KMeans를 활용한 대여소 선정 및 중첩 대여소 제거

**문제정의**

- 왼편 그림과 같은 방식으로 대여소를 추천할 경우 사용자는 제한된 방향의 대여소를 중첩 추천 받는 문제 발생
- 여러 대여소를 추천받는다 할지라도 사용자가 이용하는 경로의 개수는 제한적임
- 오른쪽 지도와 같이 여러 방향에 위치한 대여소를 추천한다면 다양한 경로를 개척할 수 있음
- 사용자가 다양한 경로를 이용해 집 주변을 탐방 할 수 있도록 여러 방향에 위치한 대여소 추천 방법 필요
   <div align='center'>
...
- Kmeans를 2차에 걸쳐 적용해 다양한 방향에 위치한 대여소 확보 및 중첩 대여소 제거
- 1차 클러스터링을 통해 출발 대여소 기준 동서남북 네 그룹으로 구분
- 그룹 지정된 대여소 별 2차 클러스터링 진행 및 중복 대여소 제거
  <div align='center'>
...
### 3. 예상시간 및 예상거리 계산을 위한 DBSCAN 활용

**문제정의**

- 데이터에 포함된 이용시간과 이용거리를 활용해 대여소 간 예상시간, 예상거리를 계산하는데 어려움이 존재
- 일례로 여의나루역 대여소에서 여의도역 대여소까지 따릉이로 이동 시 이용거리 1km 내외 이용 시간 7분 내외로 이동 가능
- 하지만 해당 대여소 간 대여기록 범위는 이용시간 3 ~ 120분, 이용거리 1 ~15km 범위로 존재
  <div align='center'>
  <img width='600px' src='ReadMeImg/dbscan1.png'>
  </div>
- 따릉이 이용 목적이 다양한 관계로 해당 분포를 오류로 판단 할 수 없음. 따라서 여러 이용 목적 중 대여소 간 이동 수단으로 따릉이를 이용한 경우를 찾아야함

**문제해결**

- DBSCAN을 활용해 대여소와 대여소 이동 수단으로 따릉이를 이용하는 경우를 추출
- DBSCAN을 활용한 이유는 다음과 같음
  - 따릉이를 대여소 간 이동 수단으로 이용하는 경우, 이동 시 효율적인 경로를 이용하므로 비슷한 거리, 시간대로 따릉이를 이용
  - 해당 사용자들의 대여기록은 아래 그래프와 같이 밀집된 형태로 나타남
    <div align='center'>
    <img width='500px' src='ReadMeImg/dbscan2.png'>
    </div>
- DBSCAN으로 추출한 대여기록을 가중평균하여 예상 시간과 거리를 산출
