...
### 1. 상관관계 분석을 통해 경로에 오르막길이 포함된 대여소 제거

**문제 정의**

- 오르막길이 있는 경로를 이용할 경우 체력적인 부담으로 인해 자전거 여행을 온전히 즐기지 못할 가능성 존재

- 따라서 경로 상 오르막길이 있을 것으로 판단되는 대여소를 식별해 제거해야할 필요성을 인식

**문제 해결**

- 대여소 간 이동 기록 차이를 이용해 오르막길이 있을것으로 판단되는 대여소 식별하였음
...
- 같은 경로임에도 비율 차이가 발생하는 이유를 찾기 위해 EDA를 수행한 결과 이동 비율과 대여소 고도 차이 간 상관 관계를 식별

- 상관관계 식별을 위해 사용한 식은 다음과 같음

- 비율 = $log_{2}(\frac{A→B \; 대여건수}{B→A \; 대여건수})$ &nbsp;|&nbsp; 고도차 = 목적지 고도 - 출발지 고도

- 초록색 영역은 출발 대여소→ 도착 대여소 경로가 오르막인 경우, 주황색 영역은 출발 대여소 → 도착 대여소 경로가 내리막인 경우를 의미함
...
- 따라서 대여소 간 비율을 계산한 뒤 비율이 -0.4 미만인 대여소를 경로 상 오르막길이 있을것으로 예상 판단하여 제거하였음
...
- 왼편 그림과 같은 방식으로 대여소를 추천할 경우 사용자는 제한된 방향의 대여소를 중첩 되게 추천 받음.

- 오른쪽 지도와 같이 사용자가 다양한 경로를 이용해 집 주변을 탐방 할 수 있도록 대여소를 추천하는 방법 필요
   <div align='center'>
...
- 일례로 여의나루역 대여소에서 여의도역 대여소까지 따릉이로 이동 시 이용거리 1km 내외 이용 시간 7분 내외로 이동 가능. 하지만 해당 대여소 간 대여기록의 범위는 이용시간 3 ~ 120분, 이용거리 1 ~15km 범위로 존재
...
- DBSCAN을 활용한 이유는 다음과 같음
  ...
  - 따라서 해당 사용자들의 대여기록은 아래 그래프와 같이 밀집된 형태로 나타남

    <div align='center'>
    <img width='600px' src='ReadMeImg/dbscan2.png'>
    </div>
