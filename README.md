...
<div align='center' size='20px'> <h2> 따릉이 이용해 주변 동네의 새로운 모습을 발견하세요!</h2>
...
서울 공공데이터 포털에서 확보한 `2021년 따릉이 대여기록` 데이터를 활용했습니다. 해당 데이터는 Raw 3,000만건 column 6개(`자전거 번호`, `대여날짜`, `대여 대여소 번호`, `반납 대여소 번호`,`대여 시간`, `이동거리`)로 구성되었습니다.
...
최종 단계는 중첩된 대여소들을 하나로 통합하는 단계입니다.

출발지에서 목적지로 향하는 방향에 따라 여행 경로가 결정되기 때문에 동일한 경로를 이용하는 대여소는 사실상 중복된 대여소와 같습니다. 중첩된 대여소를 제거하고 방향을 보존함으로써 사용자에 더 좋은 목적지를 추천할 수 있습니다.

중첩된 대여소를 찾고 제거하는 과정은 두 번의 K-menas Clustering을 통해 진행됩니다. 클러스터링을 두 번 적용하는 이유는 출발 대여소를 기준으로 같은 방향에 위치한 대여소들 중에 중첩된 대여소를 쉽게 찾아낼 수 있기 때문입니다.

아래 지도를 보면 알 수 있듯이 출발 대여소 기준으로 같은 방향에 있는 대여소들이 군집을 형성하고 있습니다. 1차로 군집된 대여소들에 다시 한번 클러스터링을 적용함으로서 특정 군집에 속한 대여소들의 세부 방향을 정할 수 있습니다. 즉 1차 클러스터링으로 큰 방향성을 정한 뒤 2차 클러스터링으로 세부적인 방향을 정하게 됩니다.
...
1차 클러스터링의 경우 출발 대여소와의 직선거리를 기준으로 구분한 다음 클러스터링 기법을 적용합니다. 예로들어 출발 대여소와 2 ~ 4km 사이에 있는 대여소를 추출한 다음 해당 대여소들에 대해 클러스터링을 진행합니다. 이런 방식으로 4~6, 6~8, 8~10km 거리에 있는 대여소에도 클러스터링을 진행합니다.

거리를 나눠 세부적으로 클러스터링을 처리함으로서 추출된 목적지가 다양한 방향에 위치하게 되며, 거리 또한 2km 단위로 보존되므로, 사용자에게 다양한 방향과 거리에 위치한 목적지를 제공 할 수 있습니다.

2단계는 군집 내 대여소들의 세부 방향을 정함과 동시에 중첩된 대여소를 하나로 통합하는 과정입니다. 1차로 라벨링 된 대여소들을 불러와 다시 클러스터링을 적용합니다.
